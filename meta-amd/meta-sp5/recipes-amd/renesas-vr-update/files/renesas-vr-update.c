...
int disable_packet_capture(void)
{
	...
	if (ret < 0) {
		printf("%s:Read to DMA Address Register failed with ret value \n",__func__,ret);
		return FAILURE;
	}
...
int check_available_nvm_slots(u_int16_t nvm_slot_addr)
{
	...
	if (ret > 0) {
		...
		printf("Read to DMA Address Register failed with ret value \n",ret);
		return FAILURE;
    }
...
int gen2_device_id_validation(void)
{
	...
	if (ret < 0) {
		printf("Read device_id from the device failed with ret value \n",ret);
		return FAILURE;
	}
...
int device_revision_verification(void)
{
	...
	if (ret > 0) {
		...
		printf("Read device revision from the device failed with ret value \n",ret);
		return FAILURE;
	}
...
int main(int argc, char *argv[])
{
	...
	if (argc != MAX_CMD_LINE_ARGUMENT) {
		printf("Usage: renesas-vr-update [I2CBUS] [I2C_ADDR] [UPDATE_FILE] [RAA229613/RAA229625/ISL68220] \n");
		return (-1);
	}

	vr_context.i2c_bus = atoi(argv[1]);
	vr_context.i2c_slave_addr = strtoul(argv[2], NULL, 16);
	vr_context.update_file_path = argv[3];

	if ((strncmp(argv[4], "RAA229613", 9) == 0) || (strncmp(argv[4], "RAA229625", 9) == 0))
		strncpy(vr_context.gen, "GEN3", 4);
	else if (strncmp(argv[4], "ISL68220", 9) == 0)
