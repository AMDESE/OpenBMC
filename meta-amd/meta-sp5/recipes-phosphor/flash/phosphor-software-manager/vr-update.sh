...
CRC=$(sed '8!d' $MANIFEST_FILE | awk -F= '{print $NF}')
...
if [ "$Manufacturer" == "Renesas" ]; then
    ...
    MODEL=$(sed '7!d' $MANIFEST_FILE | awk -F= '{print $NF}')
    /usr/bin/renesas-vr-update.sh $PROCESSOR $SLAVEADDR "$IMAGE_FILE" $Manufacturer $MODEL $CRC
    if [ $? -eq 0 ]; then
