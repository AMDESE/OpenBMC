...
cp -rp init shutdown update amd-whitelist whitelist bin sbin usr lib etc var run/initramfs
...
then
	...
	get_fw_env_var fw_version >> $optfile
fi
...
if grep -w preserve $optfile
then
	echo "Request to preserv firmware version."
	touch $trigger
	preserve_fw_ver=--preserve-fw_version
else
	preserve_fw_ver=--no-preserve-fw_version
fi
...
then
	...
	then
		...
		$update --no-restore-files $preserve_fw_ver $do_save
